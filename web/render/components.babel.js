        ...
        const scoreAndRecommend = (situation, periods, allPersonas) => {
            ...
            const scores = {};
            for (const [id, kws] of Object.entries(PERSONA_KEYWORDS_MAP)) { scores[id] = kws.reduce((a, kw) => a + (text.includes(kw) ? 1 : 0), 0); }
            for (const period of (periods||[])) { for (const id of (PERIOD_BONUSES_MAP[period]||[])) { scores[id] = (scores[id]||0) + 2; } }
            const ranked = Object.entries(scores).filter(([,s]) => s > 0).sort(([,a],[,b]) => b-a).map(([id]) => id);
            const ids = ranked.length >= 2 ? ranked : [...new Set([...ranked, 'munger','caretaker','rebel'])];
            return ids.slice(0,4).map(id => allPersonas.find(p => p.id === id)).filter(Boolean);
        };
        const PersonaRecommendModal = ({ isOpen, onClose, situation, periods, personas, selectedIds, onConfirm }) => {
            ...
            useEffect(() => {
                ...
                fetch('/recommend-personas', { method: 'POST', headers: { 'Content-Type': 'application/json' }, credentials: 'include', body: JSON.stringify({ situation, periods: periods||[], persona_ids: personas.map(p => p.id) }) })
                    .then(r => r.json())
                    .then(data => { const ids = Array.isArray(data.ids) && data.ids.length >= 2 ? data.ids : null; const recs = ids ? ids.map(id => personas.find(p => p.id === id)).filter(Boolean).slice(0,4) : scoreAndRecommend(situation, periods, personas); applyRecs(recs); })
                    .catch(() => applyRecs(scoreAndRecommend(situation, periods, personas)));
            }, [isOpen]);
            if (!isOpen) return null;
