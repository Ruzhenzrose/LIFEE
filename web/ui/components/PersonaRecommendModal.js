    ...
    >
        ...
        {persona.voice && (
            <p className="text-[10px] text-[#5D576B]/60 italic leading-relaxed line-clamp-2">
                "{persona.voice.slice(0, 72)}{persona.voice.length > 72 ? '…' : ''}"
            </p>
        )}
    </button>
);
...
const PersonaRecommendModal = ({ isOpen, onClose, situation, periods, personas, selectedIds, onConfirm }) => {
    ...
    const [generated, setGenerated] = useState([]);   // brand-new AI-generated personas
    ...
    const handleConfirm = () => {
        const mergedIds = [...new Set([...selectedIds, ...picks])];
        // include full generated persona objects for picked generated ones
        const pickedGenerated = generated.filter(p => picks.includes(p.id));
        onConfirm({ ids: mergedIds, generatedPersonas: pickedGenerated });
    };

    const allEmpty = !loading && recommended.length === 0;
        ...
        <div className="fixed inset-0 z-[100] flex items-end md:items-center justify-center p-0 md:p-4" onClick={onClose}>
            ...
            >
                ...
                <div className="flex-shrink-0 px-8 pb-8 md:px-10 md:pb-10 pt-4 border-t border-[#F0EDEA] space-y-3">
                    ...
                    <button
                        onClick={() => onConfirm({ ids: selectedIds, generatedPersonas: [] })}
